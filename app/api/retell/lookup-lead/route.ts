 ...
 * Returns key info the agent can use in conversation (website URL, services, reviews, etc.)
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(`[lookup] Searching for: name="${business_name || ""}" phone="${phone_number || ""}"`);
    ...
    // If still not found, try partial match on all leads
    if (!lead && business_name) {
      ...
      lead = allLeads.find(l => {
        ...
        // Check if any word in the search matches any word in the business name
